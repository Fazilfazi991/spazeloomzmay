...
async function main() {
  ...
  for (const asset of assets) {
    // extract original filename by removing the hash, e.g., perfume-1-DZK7-MYH.jpg -> perfume-1.jpg
    const filename = asset.split('/').pop();
    const cleanFilename = filename.replace(/-[a-zA-Z0-9_-]+\.(jpg|jpeg|png|webp|svg)$/, '.$1');
